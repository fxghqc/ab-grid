...
(function (awk) {
    ...
    (function (grid) {
        ...
        var GridOptionsWrapper = (function () {
            ...
            GridOptionsWrapper.prototype.isFilterHideNotAvailable = function () { return isTrue(this.gridOptions.filterHideNotAvailable); };
            ...
            GridOptionsWrapper.prototype.getPagingTemplate = function () { return this.gridOptions.pagingTemplate; };
    ...
    (function (grid) {
        ...
        var ColumnController = (function () {
            ...
            ColumnController.prototype.setPinnedColumnCount = function (count) {
                ...
                this.updateModel();
                var event = new grid.ColumnChangeEvent(grid.ColumnChangeEvent.TYPE_COLUMN_GROUP_OPENED).withPinnedColumnCount(count);
...
(function (awk) {
    ...
    (function (grid) {
        ...
        var RenderedCell = (function () {
            ...
            RenderedCell.prototype.defaultEditor = function () {
                var eInput = document.createElement('input');
                eInput.type = 'text';
                _.addCssClass(eInput, 'ag-cell-edit-input');
                return eInput;
            };
            RenderedCell.prototype.editCell = function (element, select) {
                var value = this.getValue();
                if (value !== null && value !== undefined) {
                    element.value = value;
                }
                element.style.width = (this.column.actualWidth - 14) + 'px';
                this.vGridCell.appendChild(element);
                element.focus();
                if (select) {
                    element.select();
                }
                this.setupEditorEvent(element);
            };
            RenderedCell.prototype.setupEditorEvent = function (element) {
                var that = this;
                var blurListener = function () {
                    that.stopEditing(element, blurListener);
                };
            ...
            RenderedCell.prototype.startEditing = function () {
                ...
                var cellEditor = colDef.cellEditor;
                if (typeof cellEditor === 'function') {
                    this.editCell(cellEditor(), false);
                }
                else {
                    this.editCell(this.defaultEditor(), true);
                }
            };
...
(function (awk) {
    ...
    (function (grid) {
        ...
        var template = '<div class="ag-paging-panel">' +
            ...
            '</span>' +
            '</div>';
        var templateJustified = '<div class="ag-paging-panel row">' +
            '<span id="pageRowSummaryPanel" class="ag-paging-row-summary-panel-x col-md-3">' +
            '[P1] ' +
            '<span id="firstRowOnPage"></span>' +
            ' [P2] ' +
            '<span id="lastRowOnPage"></span>' +
            ' [P3]' +
            '</span>' +
            '<span class= "ag-paging-page-summary-panel col-md-6 col-sm-12 col-xs-12">' +
            '<button class="ag-paging-button" id="btFirst">[FIRST]</button>' +
            '<button class="ag-paging-button" id="btPrevious">[PREVIOUS]</button>' +
            '[P4] ' +
            '<span id="current"></span>' +
            ...
            '<button class="ag-paging-button" id="btLast">[LAST]</button>' +
            '</span>' +
            '<span id="pageTotalSummaryPanel" class="ag-paging-total-summary-panel col-md-3 text-right">' +
            '[P6] ' +
            '<span id="recordCount"></span>' +
            ' [P7] ' +
            '<span id="total"></span>' +
        ...
        var PaginationController = (function () {
            ...
            PaginationController.prototype.setTotalLabels = function () {
                switch (this.templateType) {
                    case 'justify':
                        this.setTotalLabelsJustify();
                        break;
                    default:
                        this.setTotalLabelsDefault();
                }
            };
            PaginationController.prototype.setTotalLabelsJustify = function () {
                if (this.foundMaxRow) {
                    this.ePageTotalSummaryPanel.style.display = 'initial';
                    this.lbTotal.innerHTML = this.totalPages.toLocaleString();
                    this.lbRecordCount.innerHTML = this.rowCount.toLocaleString();
                }
                else {
                    this.ePageTotalSummaryPanel.style.display = 'none';
                }
            };
            PaginationController.prototype.setTotalLabelsDefault = function () {
                if (this.foundMaxRow) {
                    this.lbTotal.innerHTML = this.totalPages.toLocaleString();
                    this.lbRecordCount.innerHTML = this.rowCount.toLocaleString();
                }
            ...
            PaginationController.prototype.createTemplate = function () {
                // TODO: use zh template
                var type = this.gridOptionsWrapper.getPagingTemplate();
                var localeTextFunc = this.gridOptionsWrapper.getLocaleTextFunc();
                var result;
                switch (type) {
                    case 'justify':
                        this.templateType = 'justify';
                        result = templateJustified
                            .replace('[P1]', localeTextFunc('p1', ''))
                            .replace('[P2]', localeTextFunc('p2', 'to'))
                            .replace('[P3]', localeTextFunc('p3', 'row'))
                            .replace('[P4]', localeTextFunc('p4', 'page'))
                            .replace('[P5]', localeTextFunc('p5', ''))
                            .replace('[P6]', localeTextFunc('p6', 'total'))
                            .replace('[P7]', localeTextFunc('p7', 'rows'))
                            .replace('[P8]', localeTextFunc('p8', 'pages'))
            ...
            PaginationController.prototype.setupComponents = function () {
                ...
                this.ePageTotalSummaryPanel = this.eGui.querySelector('#pageTotalSummaryPanel');
                var that = this;
